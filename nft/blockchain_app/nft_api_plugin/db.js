...
const saveNFTHistory = async (db, decodedBlock, registeredModules, channel) => {
  decodedBlock.payload.map(async (trx) => {
    ...
    if (module.name === 'nft') {
      let dbKey; let savedHistory; let base32Address; let nftHistory; let
        encodedNFTHistory;
...
const getTransactionAssetSchema = (
  ...
  const txAssetSchema = registeredSchema.transactionsAssets.find(
    (assetSchema) => assetSchema.moduleID === transaction.moduleID && assetSchema.assetID === transaction.assetID,
  );
  if (!txAssetSchema) {
    throw new Error(
